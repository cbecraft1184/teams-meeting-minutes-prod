...
import { autoGenerateMinutes } from "./minutesGenerator";
...
async function processMinutesGenerationJob(job: Job): Promise<void> {
  ...
  try {
    // Get transcript content from the meeting if available
    const meeting = await db.query.meetings.findFirst({
      where: (m, { eq }) => eq(m.id, meetingId),
    });
    
    // Call the real AI minutes generator service
    // The autoGenerateMinutes function handles creating the minutes record
    // so we need to delete the placeholder we created above
    if (minutes) {
      await db.delete(meetingMinutes).where(eq(meetingMinutes.id, minutes.id));
    }
    
    // autoGenerateMinutes will create the minutes record and action items
    // Pass any transcript content if available (from enrichment)
    await autoGenerateMinutes(meetingId, meeting?.transcriptUrl || null);
